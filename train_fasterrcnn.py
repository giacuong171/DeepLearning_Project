...
def get_args():
    ...
    parser.add_argument(
        "--data_path", "-d", type=str, default="my_pascal_voc", help="Path to dataset"
    )
    parser.add_argument("--year", "-y", type=str, default="2012")
    parser.add_argument(
        "--num_epochs", "-n", type=int, default=100, help="Number of epochs"
    )
    ...
    parser.add_argument(
        "--momentum", "-m", type=float, default=0.9, help="Momentum for optimizer"
    )
    parser.add_argument(
        "--log_folder",
        "-p",
...
def train(args):
    ...
    train_transform = Compose(
        [
            RandomAffine(
                degrees=(-5, 5), translate=(0.15, 0.15), scale=(0.85, 1.15), shear=10
            ),
    ...
    for epoch in range(start_epoch, args.num_epochs):
        ...
        for iter, (images, labels) in enumerate(progress_bar):
            ...
            progress_bar.set_description(
                "Epoch {}/{}. Loss {:0.4f}".format(
                    epoch + 1, args.num_epochs, mean_loss
                )
            )
            writer.add_scalar(
                "Train/Loss", mean_loss, epoch * num_iters_per_epoch + iter
            )
